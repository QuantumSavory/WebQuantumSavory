...
- `src/components/tags/` contains the reusable metadata-driven tag/query constructor, target selector, structured results list, and the composed Tags & Queries tools panel; `src/composables/useTagExplorer.js` owns its catalog loading, abortable requests, mutation refreshes, and transient runtime state.
...
- Build tag/query forms from the `/tag_types` catalog rather than hard-coded tag shapes. Keep named fields fixed and documented, restrict dynamic general fields to advertised signatures, use the shared Function editor for policy-gated custom predicates, and render list/query responses through the shared structured results component with explicit disclosure of rendered text and string IDs.
