...
- Keep the simulation sidebar anchored at its fixed right offset and resize it only from its left boundary. Persist its validated width in `rightSidebar_width`, clamp it to retain usable main-panel space, preserve it while the sidebar is hidden, and keep both pointer and keyboard resizing available. Drive the sidebar and its external collapse control from the shared `--app-shell-sidebar-width` token so shell geometry stays synchronized.
