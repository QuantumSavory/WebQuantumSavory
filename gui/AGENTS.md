...
- Physical badges are session-only, default on, ignore pointer input, and place distance above delay at the Turf half-length point. A manual delay hides distance and reports distance/refractive index as `n/a` in the controls without deleting their dormant overrides; physical resolution still returns numeric distance/index for backend custom-function context while only the delay formula changes. Curve handles appear only for a selected, unlocked physical edge while curve mode is active; new handles are smooth and clicks cycle smooth → sharp → delete.
