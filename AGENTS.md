...
- Keep annotation validation and geometry in `gui/src/utils/annotationGeometry.js`; map components must preserve selected annotation object identity, insert rectangle layers before every edge layer, keep node markers above annotation HTML, and release annotation layers, sources, markers, and listeners during project transitions and unmount.
