...
- Custom-function context is backend-owned and lexical. Build the node-name-to-1-based-index mapping once from the ordered validated nodes for each protocol launch, pass it through node, edge, and floating conversion, and keep the supported bindings plus their construction centralized beside function conversion. `nodeid(name::String)` is available to every custom Lambda, while `self` is bound only for node protocols. Edge protocols additionally bind `length` in meters, `delay` in seconds, dimensionless `refractive_index`, and the source/target one-based IDs as `node_a`/`node_b`; the three physical values are `nothing` on virtual edges, and `length` intentionally shadows `Base.length`. Lambda variables remain raw and are instantiated separately at each assignment. Duplicate node names intentionally use `Dict` last-name-wins semantics, and an unknown `nodeid` lookup fails with the ordinary Julia `KeyError` when the function runs.
...
- Physical edges alone define the simulator's `SimpleGraph`. Reject duplicate unordered physical endpoint pairs; validate their resolved nonnegative finite `data.distanceMeters` and `data.propagationDelaySeconds` plus positive finite `data.refractiveIndex`; and pass the resulting symmetric delay lookup as both `classical_delay` and `quantum_delay` to `RegisterNet`. Missing distance/index remain `nothing` for legacy payloads, while missing legacy delay retains the established zero default. Virtual edges remain in validated edge metadata only so protocols whose catalog type permits virtual placement can still be constructed; reject other virtual-edge protocols.
