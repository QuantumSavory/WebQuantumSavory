...
- Preserve runtime/export parity for custom-function context without adding stored-project fields. Generated scripts define the deterministic node-name map and `nodeid` helper before register and background construction, bind node-only `self` and edge-only physical/endpoint values lexically around custom functions, and instantiate context-dependent Variables at each protocol or background assignment; never introduce shared mutable context state.
- Preserve runtime/export parity for numeric expressions with the same lexical
  ...
  and Variable-backed expressions are evaluated with the concrete protocol or
  background assignment, and each expression Variable is evaluated
  independently per assignment.
